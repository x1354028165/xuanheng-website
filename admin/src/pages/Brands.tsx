...
export default function Brands() {
  ...
  const columns = [
    ...
    { title: t('brands.category'), dataIndex: 'category', width: 120 },
    { title: t('brands.logo'), dataIndex: 'logoUrl', width: 100, render: (v: string) => v ? <img src={v} alt="logo" style={{ height: 24 }} /> : '-' },
    { title: t('brands.capabilities'), dataIndex: 'capabilities', ellipsis: true },
    {
    ...
    <div>
      ...
      <Drawer title={editingId ? t('common.edit') : t('common.create')} open={drawerOpen} onClose={() => setDrawerOpen(false)} width={520}
        ...
        <Form form={form} layout="vertical">
          <Form.Item name="name" label={t('brands.brandName')} rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="category" label={t('brands.category')}><Input /></Form.Item>
          <Form.Item name="logoUrl" label={t('brands.logo')}><Input placeholder="URL" /></Form.Item>
          <Form.Item name="capabilities" label={t('brands.capabilities')}><Input placeholder="e.g. BACnet, Modbus, KNX" /></Form.Item>
        </Form>
      </Drawer>
    </div>
  );
