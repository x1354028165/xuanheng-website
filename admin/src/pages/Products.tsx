...
export default function Products() {
  ...
  return (
    <div>
      <Typography.Title level={4}>{t('menu.products')}</Typography.Title>
      <Table dataSource={data} columns={columns} rowKey="id" loading={loading} pagination={false} />
      <Drawer title={t('products.editProduct')} open={drawerOpen} onClose={() => setDrawerOpen(false)} width={540}
        extra={<Button type="primary" onClick={handleSave} loading={uploading}>{t('common.save')}</Button>}>
        <Form form={form} layout="vertical">
          <Form.Item name="title" label={t('common.title')} rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="tagline" label={t('products.tagline')}><Input /></Form.Item>
          <Form.Item name="description" label={t('common.description')}><Input.TextArea rows={5} /></Form.Item>
          <Form.Item label="封面图">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {coverPreview && (
                <img src={coverPreview.startsWith('http') ? coverPreview : coverPreview}
                  alt="封面" style={{ width: '100%', maxHeight: 200, objectFit: 'cover', borderRadius: 8, border: '1px solid #e2e8f0' }} />
              )}
