...
import { Table, Input, Button, Drawer, Form, Space, Popconfirm, Typography, message, Switch, Tag } from 'antd';
import { PlusOutlined } from '@ant-design/icons';
import { useTranslation } from 'react-i18next';
import api from '../lib/axios';

const API_URL = '/content-manager/collection-types/api::notification-config.notification-config';
...
export default function NotificationConfig() {
  ...
  const columns = [
    { title: t('notification.configKey'), dataIndex: 'configKey', ellipsis: true },
    { title: t('notification.webhookUrl'), dataIndex: 'webhookUrl', ellipsis: true },
    { title: t('notification.emailTo'), dataIndex: 'emailTo', width: 160 },
    { title: t('notification.isEnabled'), dataIndex: 'isEnabled', width: 80, render: (v: boolean) => <Tag color={v ? 'green' : 'red'}>{v ? t('notification.enabled') : t('notification.disabled')}</Tag> },
    {
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        <Typography.Title level={4} style={{ margin: 0 }}>{t('menu.notificationConfig')}</Typography.Title>
      ...
      <Drawer title={editingId ? t('common.edit') : t('common.create')} open={drawerOpen} onClose={() => setDrawerOpen(false)} width={520}
        ...
        <Form form={form} layout="vertical">
          <Form.Item name="configKey" label={t('notification.configKey')} rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="webhookUrl" label={t('notification.webhookUrl')}><Input /></Form.Item>
          <Form.Item name="emailTo" label={t('notification.emailTo')}><Input /></Form.Item>
          <Form.Item name="isEnabled" label={t('notification.isEnabled')} valuePropName="checked"><Switch /></Form.Item>
          <Form.Item name="description" label={t('notification.description')}><Input.TextArea rows={2} /></Form.Item>
        </Form>
      </Drawer>
    </div>
  );
