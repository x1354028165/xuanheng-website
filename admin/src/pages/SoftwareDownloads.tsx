...
const API_URL = '/api/software-downloads';
...
export default function SoftwareDownloads() {
  ...
  const columns = [
    { title: t('softwareDownloads.name'), dataIndex: 'name', ellipsis: true },
    { title: t('softwareDownloads.version'), dataIndex: 'version', width: 100 },
    { title: t('softwareDownloads.platform'), dataIndex: 'platform', width: 120 },
    {
      title: t('softwareDownloads.releaseDate'),
      dataIndex: 'releaseDate',
      width: 120,
      render: (v: string) => v ? dayjs(v).format('YYYY-MM-DD') : '-',
    },
    { title: t('softwareDownloads.sortOrder'), dataIndex: 'sortOrder', width: 80 },
    {
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        <Typography.Title level={4} style={{ margin: 0 }}>{t('menu.softwareDownloads')}</Typography.Title>
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="name" label={t('softwareDownloads.name')} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="version" label={t('softwareDownloads.version')} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="platform" label={t('softwareDownloads.platform')}>
            <Input placeholder="Windows / macOS / Linux / Android / iOS" />
          </Form.Item>
          <Form.Item name="description" label={t('softwareDownloads.description')}>
            <Input.TextArea rows={4} />
          </Form.Item>
          <Form.Item name="fileUrl" label={t('softwareDownloads.fileUrl')} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="releaseDate" label={t('softwareDownloads.releaseDate')}>
            <DatePicker style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="sortOrder" label={t('softwareDownloads.sortOrder')}>
