...
export default function PageContent() {
  ...
  const columns = [
    { title: t('pageContent.pageKey'), dataIndex: 'pageKey', ellipsis: true },
    { title: t('pageContent.blockKey'), dataIndex: 'blockKey', width: 150 },
    { title: t('pageContent.description'), dataIndex: 'description', ellipsis: true },
    {
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        <Typography.Title level={4} style={{ margin: 0 }}>{t('menu.pageContent')}</Typography.Title>
      ...
      <Drawer title={editingId ? t('common.edit') : t('common.create')} open={drawerOpen} onClose={() => setDrawerOpen(false)} width={520}
        ...
        <Form form={form} layout="vertical">
          <Form.Item name="pageKey" label={t('pageContent.pageKey')} rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="blockKey" label={t('pageContent.blockKey')}><Input /></Form.Item>
          <Form.Item name="content" label={t('pageContent.content')}><Input.TextArea rows={8} placeholder={t('pageContent.jsonPlaceholder')} /></Form.Item>
          <Form.Item name="description" label={t('pageContent.description')}><Input.TextArea rows={2} /></Form.Item>
        </Form>
      </Drawer>
    </div>
  );
